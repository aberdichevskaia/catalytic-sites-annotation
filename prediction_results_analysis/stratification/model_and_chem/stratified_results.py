...
- Model source: PDB vs AF (inferred from Sequence_ID prefix before "_chain")

Outputs (in --out_dir):
  - metrics_by_ec_top.csv
  - metrics_by_chemotype.csv
  - metrics_by_model_source.csv
  - barplot_aucpr_by_ec_top__with_ci_clean.png
  - barplot_aucpr_by_chemotype__with_ci_clean.png
  - barplot_aucpr_by_model_source__with_ci_clean.png
  - per_chain_debug.csv
"""
...
def is_pdb_identifier(s: str) -> bool:
    # PDB id: exactly 4 alphanumeric chars (case-insensitive)
    return (len(s) == 4) and s.isalnum()


def parse_model_source(seq_id: Any) -> Optional[str]:
    """
    Uses id_chain format: e.g. "1abc_A", "A0A1L8G2K9_A"
    - take prefix before "_"
    - if it's PDB-like (len==4 & alnum) -> "PDB"
    - else -> "AF"
    """
    if seq_id is None:
        return None
    s = normalize_id(seq_id)
    base = s.split("_", 1)[0]
...
def _resolve_out(out_dir: str, name_or_path: str) -> str:
    """If user passes absolute path -> keep it; else save inside out_dir."""
...
def barplot_with_ci_clean(values, labels, ci_lo, ci_hi, ylabel, title, out_path, ylim=(0.0, 1.0)):
    ...
    ax.bar(x, values, color="#7BC8F6", edgecolor='black')
    ...
    fig.tight_layout()
    fig.savefig(out_path, dpi=1000, format="png")
    plt.close(fig)
...
def main():
    ...
    ap.add_argument("--out-dir", dest="out_dir", type=str, required=True, help="Output directory.")

    # optional inputs (required only if corresponding stratification is enabled)
    ap.add_argument("--dataset-csv", dest="dataset_csv", type=str, default=None,
                    help="Path to dataset.csv with Sequence_ID, EC_number. Required if --ec.")
    ap.add_argument("--split-txts", dest="split_txts", type=str, nargs="*", default=None,
                    help="Paths to split*.txt label files (used to recover sequences). Required if --chemotype.")

    # stratification toggles (robust: works on Python 3.7+)
    g = ap.add_mutually_exclusive_group()
    g.add_argument("--ec", dest="do_ec", action="store_true", help="Run EC top-level stratification.")
    g.add_argument("--no-ec", dest="do_ec", action="store_false", help="Disable EC top-level stratification.")
    ap.set_defaults(do_ec=True)

    g = ap.add_mutually_exclusive_group()
    g.add_argument("--chemotype", dest="do_chemotype", action="store_true", help="Run chemotype stratification.")
    g.add_argument("--no-chemotype", dest="do_chemotype", action="store_false", help="Disable chemotype stratification.")
    ap.set_defaults(do_chemotype=True)

    g = ap.add_mutually_exclusive_group()
    g.add_argument("--model-source", dest="do_model_source", action="store_true", help="Run PDB vs AF stratification.")
    g.add_argument("--no-model-source", dest="do_model_source", action="store_false", help="Disable PDB vs AF stratification.")
    ap.set_defaults(do_model_source=True)

    g = ap.add_mutually_exclusive_group()
    g.add_argument("--debug-csv", dest="do_debug_csv", action="store_true", help="Write per-chain debug CSV.")
    g.add_argument("--no-debug-csv", dest="do_debug_csv", action="store_false", help="Do not write per-chain debug CSV.")
    ap.set_defaults(do_debug_csv=True)

    # other params
    ap.add_argument("--k", type=int, default=5, help="k for MP@k context columns (default: 5).")
    ap.add_argument("--n-boot", dest="n_boot", type=int, default=200,
    ...
    ap.add_argument("--progress-every", dest="progress_every", type=int, default=0,
                    help="Print bootstrap progress every N iterations (0 disables).")

    # output filenames (saved inside --out-dir unless absolute path)
    ap.add_argument("--out-debug", dest="out_debug", type=str, default="per_chain_debug.csv",
                    help="Filename for per-chain debug CSV.")
    ap.add_argument("--out-metrics-ec", dest="out_metrics_ec", type=str, default="metrics_by_ec_top.csv",
                    help="Filename for EC metrics CSV.")
    ap.add_argument("--out-plot-ec", dest="out_plot_ec", type=str, default="barplot_aucpr_by_ec_top__with_ci_clean.png",
                    help="Filename for EC AUCPR barplot PNG.")
    ap.add_argument("--out-metrics-chemotype", dest="out_metrics_chemotype", type=str, default="metrics_by_chemotype.csv",
                    help="Filename for chemotype metrics CSV.")
    ap.add_argument("--out-plot-chemotype", dest="out_plot_chemotype", type=str, default="barplot_aucpr_by_chemotype__with_ci_clean.png",
                    help="Filename for chemotype AUCPR barplot PNG.")
    ap.add_argument("--out-metrics-model-source", dest="out_metrics_model_source", type=str, default="metrics_by_model_source.csv",
                    help="Filename for model-source metrics CSV.")
    ap.add_argument("--out-plot-model-source", dest="out_plot_model_source", type=str, default="barplot_aucpr_by_model_source__with_ci_clean.png",
                    help="Filename for model-source AUCPR barplot PNG.")
    ...
    if args.do_ec and not args.dataset_csv:
        raise ValueError("--dataset-csv is required when --ec is enabled.")
    if args.do_chemotype and (not args.split_txts or len(args.split_txts) == 0):
        raise ValueError("--split-txts is required when --chemotype is enabled.")

    # resolve output paths
    out_debug = _resolve_out(args.out_dir, args.out_debug)
    out_metrics_ec = _resolve_out(args.out_dir, args.out_metrics_ec)
    out_plot_ec = _resolve_out(args.out_dir, args.out_plot_ec)
    out_metrics_chem = _resolve_out(args.out_dir, args.out_metrics_chemotype)
    out_plot_chem = _resolve_out(args.out_dir, args.out_plot_chemotype)
    out_metrics_src = _resolve_out(args.out_dir, args.out_metrics_model_source)
    out_plot_src = _resolve_out(args.out_dir, args.out_plot_model_source)
    ...
    # Load sequences only if needed (for chemotype)
    seq_map = {}
    if args.do_chemotype:
    ...
    model_source: List[Optional[str]] = []
    n_pos: List[int] = []
    ...
    for _id, y in zip(ids, labels):
        ...
        # model source
        if args.do_model_source:
            model_source.append(parse_model_source(_id))
        else:
            model_source.append(None)
    ...
    print(f"[INFO] do_ec={args.do_ec}, do_chemotype={args.do_chemotype}, do_model_source={args.do_model_source}")
    if args.do_ec:
    ...
    if args.do_debug_csv:
        df_chain.to_csv(out_debug, index=False)
    ...
    if args.do_ec:
        ...
        for lvl in ec_levels:
            ...
            if len(idx) == 0:
                ...
                au_ec.append(0.0); au_ec_lo.append(np.nan); au_ec_hi.append(np.nan)
                continue
            ...
            au_ec.append(float(au)); au_ec_lo.append(float(lo)); au_ec_hi.append(float(hi))

        pd.DataFrame(rows_ec).to_csv(out_metrics_ec, index=False)
        barplot_with_ci_clean(
            values=au_ec, labels=labels_ec, ci_lo=au_ec_lo, ci_hi=au_ec_hi,
            ylabel="AUCPR", title="AUCPR by EC top-level (test)", out_path=out_plot_ec
        )
        wrote += [out_metrics_ec, out_plot_ec]
    ...
    if args.do_chemotype:
        ...
        for c in chem_levels:
            ...
            if len(idx) == 0:
                ...
                au_ch.append(0.0); au_ch_lo.append(np.nan); au_ch_hi.append(np.nan)
                continue
            ...
            au_ch.append(float(au)); au_ch_lo.append(float(lo)); au_ch_hi.append(float(hi))

        pd.DataFrame(rows_ch).to_csv(out_metrics_chem, index=False)
        barplot_with_ci_clean(
            values=au_ch, labels=labels_ch, ci_lo=au_ch_lo, ci_hi=au_ch_hi,
            ylabel="AUCPR", title="AUCPR by chemotype (test)", out_path=out_plot_chem
        )
        wrote += [out_metrics_chem, out_plot_chem]

    # ---------- Stratify by model source (PDB vs AF) ----------
    if args.do_model_source:
        src_levels = ["PDB", "AF"]
        rows_src = []
        labels_src = []
        au_src, au_src_lo, au_src_hi = [], [], []

        for src in src_levels:
            mask = (df_chain["model_source"] == src).to_numpy()
            idx = np.where(mask)[0]
            labels_src.append(src)
            ...
            if len(idx) == 0:
                ...
                au_src.append(0.0); au_src_lo.append(np.nan); au_src_hi.append(np.nan)
                continue
            ...
            au_src.append(float(au)); au_src_lo.append(float(lo)); au_src_hi.append(float(hi))

        pd.DataFrame(rows_src).to_csv(out_metrics_src, index=False)
        barplot_with_ci_clean(
            values=au_src, labels=labels_src, ci_lo=au_src_lo, ci_hi=au_src_hi,
            ylabel="AUCPR", title="AUCPR by model source (PDB vs AF) (test)", out_path=out_plot_src
        )
        wrote += [out_metrics_src, out_plot_src]
